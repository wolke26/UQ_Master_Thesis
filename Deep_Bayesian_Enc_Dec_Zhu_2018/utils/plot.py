...
def plot_prediction_bayes(save_dir, target, rfield, pred_mean, pred_var, epoch, index,  plot_fn='contourf'):
    """Plot predictions at *one* test input

    Args:
        save_dir: directory to save predictions
        target (np.ndarray or torch.Tensor): (1, 20, 20)
        rfield (np.ndarray or torch.Tensor): (1, 20, 20)
        pred_mean (np.ndarray or torch.Tensor): (1, 20, 20)
        pred_var (np.ndarray or torch.Tensor): (1, 20, 20)
        epoch (int): which epoch
        index (int): i-th prediction
        plot_fn (str): choices=['contourf', 'imshow']
    """
    target, rfield, pred_mean, pred_var = to_numpy(target),to_numpy(rfield), to_numpy(pred_mean), to_numpy(pred_var)
    ...
    im1 = axs[0, 0].imshow(target[0])
    axs[0, 0].set_title('$Target$')
    fig.colorbar(im1, ax=axs[0, 0])
    
    im2 = axs[0, 1].imshow(pred_mean[0])
    axs[0, 1].set_title('$\mu_{pred}$')
    fig.colorbar(im2, ax=axs[0, 1])
    
    im3 = axs[1, 0].imshow(rfield[0])
    axs[1, 0].set_title('RField Input')
    fig.colorbar(im3, ax=axs[1, 0])
    
    im4 = axs[1, 1].imshow(abs(target[0] - pred_mean[0]))
    ...
    plt.savefig(save_dir + '/pred_at_x_epoch{}_{}.pdf'.format(epoch, index), dpi=300, bbox_inches='tight')
    plt.close(fig)
...
def plot_MC(save_dir, monte_carlo, pred_mean, pred_var, mean, n_train):
    ...
    subplots_position = ['23{}'.format(i) for i in range(0, 4)]
    ...
    data = (monte_carlo[0], pred_mean[0])
    ...
    grid = ImageGrid(fig, 222,  # as in plt.subplot(111)
                     nrows_ncols=(2, 1),
                     axes_pad=axes_pad,
                     share_all=False,
                     cbar_location="right",
