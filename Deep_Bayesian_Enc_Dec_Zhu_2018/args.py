...
class Parser(argparse.ArgumentParser):
    def __init__(self):
        ...
        self.add_argument('--post', action='store_true', default=True, help='enables post analysis mode, default is training mode')
        ...
        self.add_argument('--blocks', type=list, default=[2, 5, 2], help='list of number of layers in each dense block')
        self.add_argument('--growth-rate', type=int, default=2, help='number of output feature maps of each conv layer within each dense block')
        self.add_argument('--init-features', type=int, default=8, help='number of initial features after the first conv layer')        
        self.add_argument('--drop-rate', type=float, default=0.0, help='dropout rate')
        self.add_argument('--bn-size', type=int, default=1, help='bottleneck size: bn_size * growth_rate')
        ...
        self.add_argument('--stress', type=int, default=4200, help='num of rfields')
        self.add_argument('--ntrain', type=int, default=4200, help="number of training data")
        self.add_argument('--ntest', type=int, default=800, help="number of test data") 
        self.add_argument('--nmc', type=int, default=5000, help="number of MC test data for uncertainty propagation")
        ...
        self.add_argument('--image-size', type=int, default=[20, 20], help="image resolution")
        
        # training
        self.add_argument('--epochs', type=int, default=500, help='number of epochs to train')
        self.add_argument('--lr', type=float, default=0.03, help='learnign rate')
        self.add_argument('--lr-noise', type=float, default=0.00, help='learnign rate')
        self.add_argument('--batch-size', type=int, default= 350, help='batch size for training')
        self.add_argument('--test-batch-size', type=int, default=20, help='batch size for testing')
        self.add_argument('--mc-batch-size', type=int, default=350, help='batch size for Monte Carlo')
        ...
        self.add_argument('--ckpt-epoch', type=int, default=500, help='which epoch of checkpoints to be loaded in post mode')        
    ...
    def parse(self):
        ...
        args.run_dir = args.exp_dir + '/' + args.exp_name \
            + '/rfields{}/nsamples{}_ntrain{}_batch{}_lr{}_noiselr{}_epochs{}'.format(
                args.stress, args.n_samples, args.ntrain, args.batch_size, args.lr,
