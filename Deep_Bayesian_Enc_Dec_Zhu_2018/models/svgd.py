...
class SVGD(object):
    ...
    def _optimizers_schedulers(self, lr, lr_noise):
        """Initialize Adam optimizers and schedulers (ReduceLROnPlateau) or (CosineAnnealingLR)
        ...
        for i in range(self.n_samples):
            # set_trace()
            parameters = [{'params': [self.bayes_nn.module[i].log_beta], 'lr': lr_noise},
                    {'params': self.bayes_nn.module[i].features.parameters()}]
...
#         schedulers.append(ReduceLROnPlateau(optimizer_i,mode='min', factor=0.1, patience=10, verbose=True))
            ...
            num_steps = 20
            schedulers.append(CosineAnnealingLR(optimizer_i, T_max = num_steps, eta_min=0, last_epoch=-1))
        return optimizers, schedulers


    def train(self, epoch, logger):
        # set_trace()
        self.bayes_nn.module.train()
        mse = 0.
        for batch_idx, (input, target) in enumerate(self.train_loader):
            ...
            for i in range(self.n_samples):
                output_i = self.bayes_nn.module[i].forward(input)
                # set_trace()
                output += output_i.detach()
                log_joint_i = self.bayes_nn.module._log_joint(i, output_i, target, ntrain)
                ...
                vec_param, vec_grad_log_joint = parameters_to_vector(
                    self.bayes_nn.module[i].parameters(), both=True)
            ...
            for i in range(self.n_samples):
                vector_to_parameters(grad_theta[i],
                    self.bayes_nn.module[i].parameters(), grad=True)
                self.optimizers[i].step()
            # WEAK: no loss function to suggest when to stop or
            # approximation performance

            mse += F.smooth_l1_loss(output / self.n_samples, target).item()

        # logging
        rmse_train = np.sqrt(mse / len(self.train_loader))
        # r2_train = 1 - mse * target.numel()  / np.sum(logger['train_output_var'])
        r2_train = 1 - mse  / logger['train_output_var']

        logger['rmse_train'].append(rmse_train)
        logger['r2_train'].append(r2_train)
        logger['log_beta'].append(self.bayes_nn.module.log_beta.mean().item())
