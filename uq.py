...
class UQ(object):
    ...
    def plot_prediction_at_x(self, n_pred):
        ...
        for i in idx:
            ...
            pred_mean, pred_var = self.bnn.module.predict(input.unsqueeze(0).to(device))
            save_dir = run_dir + '/predict_at_x'
            mkdir(save_dir)
            plot_prediction_bayes(save_dir, target, input, pred_mean.squeeze(0), 
    ...
    def propagate_uncertainty(self):
        ...
        y_pred_EE, y_pred_VE, y_pred_EV, y_pred_VV = self.bnn.module.propagate(self.mc_loader)
    ...
    def plot_dist(self, num_loc):
        ...
        for _, (x_mc, t_mc) in enumerate(self.mc_loader):
            x_mc = x_mc.to(device)
            # S x B x C x H x W
            y_mc = self.bnn.module.forward(x_mc)
        ...
        for loc in range(0,6):
            ...
            for c, ax in enumerate(fig.axes):
                #Hier könnte man vllt plot verwenden
                # set_trace()
                # plt.plot(target[:, c, loc], '*', color='b', label='Monte Carlo')
                # plt.plot(pred[:, c, loc],'.', color='r', label='surrogate')
                # plt.savefig(r"C:\Users\Gloria\Documents\Masterarbeit\Solving_PDE_NeuralNetwork\cnn-surrogate-master3\cnn-surrogate-master\experiments\Bayesian\rfields\nsamplesNone_ntrain20_batch800_lr800_noiselr0.0025_epochs0.01\dist_estimate\myfile.png")
                sns.kdeplot(target[:, c, loc], color='b', label='Monte Carlo',ax=ax)
                sns.kdeplot(pred[:, c, loc], color='r', label='surrogate', ax=ax)
                
                # plt.savefig(r"C:\Users\Gloria\Documents\Masterarbeit\Solving_PDE_NeuralNetwork\cnn-surrogate-master3\cnn-surrogate-master\experiments\Bayesian\rfields\nsamplesNone_ntrain20_batch800_lr800_noiselr0.0025_epochs0.01\dist_estimate")

                ax.legend()
            # plt.savefig("./experiments\Bayesian\rfields\nsamplesNone_ntrain20_batch800_lr800_noiselr0.0025_epochs0.01\dist_estimate"+"\loc_({},_ {}).pdf".format(locations[loc][0], locations[loc][1]), dpi=300)

            # plt.savefig(r"C:\Users\Gloria\Documents\Masterarbeit\Solving_PDE_NeuralNetwork\cnn-surrogate-master3\cnn-surrogate-master\experiments\Bayesian\rfields\nsamplesNone_ntrain20_batch800_lr800_noiselr0.0025_epochs0.01\dist_estimate" + "\loc_({}, {}).pdf".format(locations[loc][0], locations[loc][1]), dpi=300)
            plt.xlabel('Field value at location '+str(locations[loc][0])+'_'+str(locations[loc][1]))
            plt.ylabel('Frequency')
            plt.savefig(dist_dir + '/loc_({},_{}).pdf'.format(locations[loc][0], locations[loc][1]), dpi=300)
            plt.close(fig)
        ...
        for p in p_list:
            ...
            for batch_idx, (input, target) in enumerate(self.mc_loader):
                ...
                if batch_idx > 0:
                    continue
                # set_trace()
                pred_mean, pred_var = self.bnn.module.predict(input.to(device))
